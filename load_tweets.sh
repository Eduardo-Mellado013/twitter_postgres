...
for file in $files; do
    ...
    python3 load_tweets.py --db "postgresql://postgres:pass@localhost:6556/postgres" --inputs "$file"
done

echo "load denormalized"
for file in $files; do
    echo "Processing denormalized file: $file"
    unzip -p "$file" | sed 's/\\u0000//g' | iconv -f utf-8 -t utf-8 -c | psql "postgresql://postgres:pass@localhost:6555" -c "COPY tweets_jsonb (data) FROM STDIN csv quote e'\x01' delimiter e'\x02';"
done
